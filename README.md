...
- **JSON 美化**：一键格式化 JSON 报文，层次清晰地展示结构化数据，便于阅读和分析
...
- **导出消息记录**：将完整消息记录导出为 TXT 文件，便于归档、分享和二次分析

### 自动化测试功能
- **自动回复功能**：支持测试用的自动回复，模拟服务端或客户端响应
- **周期发送功能**：支持定时周期性发送消息，用于压力测试或长时间稳定性测试
- **压力测试功能**：内置 TCP/UDP 高并发压力测试引擎，实时展示 QPS、延迟分位数（p50/p95/p99）、成功 / 失败 / 断连统计，支持变量模板（序号、工作线程 ID、计数器、时间戳、随机数）和 CSV 报告导出
...
7. **消息管理**
   ...
   - **JSON 美化**：当消息内容为合法 JSON 时，点击美化按钮即可格式化展示
...
9. **压力测试**
   ...
   - 填写目标地址、端口、并发客户端数、发送速率、消息内容及变量模板
   - 点击开始后，在压力测试面板实时查看 QPS、p50/p95/p99 延迟、成功 / 失败 / 断连统计
